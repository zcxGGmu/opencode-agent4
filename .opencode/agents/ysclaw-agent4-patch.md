...
`/comet` 是默认主入口，负责按 OpenSpec + Superpowers 编排完整研发生命周期。`/ysclaw-patch-plan` 和 `/ysclaw-build-patch` 是该生命周期内的结构化产物能力节点。
...
- 单独调用 `/ysclaw-*` 只用于调试某个结构化产物能力，不能替代 `/comet` 完成生产路径闭环。
