...
`direct` 只表示跳过完整设计/计划技能，不表示绕过 Agent4 产物链。修改代码前仍必须基于 `RootCauseBlueprint` 或 hotfix proposal 生成精简 `PatchPlan`；修改完成后仍必须捕获真实 `git diff` 并生成 `PatchCandidate`。

继续或开始修改前，按 `comet/reference/dirty-worktree.md` 协议处理未提交改动。若归因后发现修复范围超出 hotfix，按本文件“升级条件”处理。

**立即执行：** 按 tasks.md 逐个执行任务：

1. 调用或约束 `/ysclaw-patch-plan` 生成精简 `PatchPlan`，保存到 `openspec/changes/<name>/.comet/agent4/patch-plan.json`
2. 读取 `openspec/changes/<name>/tasks.md`，获取未完成任务列表
3. 对每个未完成任务：
   - 根据任务描述修改代码
...
4. 全部任务完成后，显式运行项目相关测试和构建命令
5. 捕获 `git diff` 并生成 `PatchCandidate`，保存到 `openspec/changes/<name>/.comet/agent4/patch-candidate.json`
6. 运行阶段守卫完成 build → verify 过渡：
