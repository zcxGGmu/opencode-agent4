...
### 3. 生成 PatchRegressionResult

轻量验证或完整验证通过后，必须补齐 Agent4 的回归证据链。没有 Agent1 回归证据时，不得进入归档。

1. 定位 build 阶段产出的 `PatchCandidate`：

```text
openspec/changes/<name>/.comet/agent4/patch-candidate.json
```

2. 运行或导入 Agent1 `patch_regression` 结果。允许使用外部 Agent1 产物，但必须能追溯到当前 `PatchCandidate`。
3. 将 Agent1 输出归一化为 `PatchRegressionResult`：

```bash
node tools/ysclaw-agent4-tools.js normalize-regression \
  <agent1-patch-regression-result.json> \
...
- 验证报告通过
- `PatchRegressionResult` 已由 Agent1 `patch_regression` 运行或导入结果生成，并通过结构约束校验
- 分支已处理
