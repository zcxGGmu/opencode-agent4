---
name: comet
description: "Comet — Agent4 核心研发工作流。推荐由 /ysclaw-agent4 委托启动，也可用 /comet 兼容入口直接启动，自动检测阶段并分发到子命令。五阶段：开启 → 深度设计 → 计划与构建 → 验证与收尾 → 归档。"
---

# Comet — Agent4 核心研发工作流
...
在 `opencode-agent4` 中，`/ysclaw-agent4` 是推荐的 Agent4 主入口，委托本 `comet` skill 编排完整研发生命周期；`/comet` 保留为兼容入口，不是旁路流程。它们在对应阶段约束结构化产物链：
...
`/ysclaw-patch-plan` 和 `/ysclaw-build-patch` 是这个生命周期中的产物能力节点。单独调用只用于调试某个产物能力，不能替代 `/ysclaw-agent4` 完成生产路径闭环。
...
| 恰好 1 个 | `/ysclaw-agent4 <描述>` 或 `/comet <描述>` | → **询问**：继续该变更 or 创建新变更 |
| 多个 | `/ysclaw-agent4 <描述>` 或 `/comet <描述>` | → **询问**：继续现有变更 or 创建新变更；若选继续 → 列出清单让用户选择 |
| 恰好 1 个 | `/ysclaw-agent4` 或 `/comet`（无描述） | → 自动选中，进入 Step 1 |
| 多个 | `/ysclaw-agent4` 或 `/comet`（无描述） | → 列出清单让用户选择 |
...
<IMPORTANT>
单次 `/ysclaw-agent4` 或 `/comet` 调用从检测到的阶段开始，退出条件满足后进入下一阶段。
...
| `/comet-build` | 3. 计划与构建 | Agent4 + Superpowers | PatchPlan、PatchCandidate、代码提交 |
| `/comet-verify` | 4. 验证与收尾 | Agent4 + Both | PatchRegressionResult、验证报告、分支处理 |
| `/comet-archive` | 5. 归档 | Agent4 + OpenSpec | VerifiedPatchPackage、Agent5 handoff、delta→main spec 同步 |
| `/comet-hotfix` | 预设路径 | Agent4 + Both | 快速修复，但仍保持 PatchPlan → VerifiedPatchPackage 产物链 |
| `/comet-tweak` | 预设路径 | Agent4 + Both | 小改动，但仍保持 PatchPlan → VerifiedPatchPackage 产物链 |

```
/ysclaw-agent4
  ↓ 自动检测
/comet-open ──→ /comet-design ──→ /comet-build ──→ /comet-verify ──→ /comet-archive
  (OpenSpec)      (Superpowers)     (PatchPlan        (PatchRegression (VerifiedPatchPackage
                                     + PatchCandidate) Result)          + Agent5 handoff)

/comet（兼容入口）执行相同生命周期编排。
...
| `build_command` | 项目构建命令。guard 默认只提示该命令，不自动执行；设置 `COMET_ALLOW_CONFIGURED_COMMANDS=1` 后才允许 guard 执行 |
| `verify_command` | 项目验证命令。verify guard 默认只提示该命令，不自动执行；未配置时回退到默认构建探测 |
