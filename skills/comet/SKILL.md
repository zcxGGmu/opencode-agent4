---
name: comet
description: "Comet — Agent4 核心研发工作流。用 /comet 启动，自动检测阶段并分发到子命令。五阶段：开启 → 深度设计 → 计划与构建 → 验证与收尾 → 归档。"
---
...
`/ysclaw-patch-plan` 和 `/ysclaw-build-patch` 是这个生命周期中的产物能力节点。单独调用只用于调试某个产物能力，不能替代 `/comet` 完成生产路径闭环。
